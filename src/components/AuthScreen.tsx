import { useState } from 'react';
import { signInWithPopup } from 'firebase/auth';
import { auth, googleProvider } from '../firebase';
import { AlertCircle, Loader } from 'lucide-react';

export default function AuthScreen() {
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);

  const translateError = (code: string) => {
    switch (code) {
      case 'auth/popup-closed-by-user':
        return 'Login cancelado.';
      case 'auth/cancelled-popup-request':
        return 'Ação cancelada pelo usuário.';
      case 'auth/operation-not-allowed':
        return 'O login com Google não está ativado no Firebase.';
      default:
        return 'Ocorreu um erro ao conectar com o Google. Tente novamente.';
    }
  };

  const handleGoogleLogin = async () => {
    setError(null);
    setLoading(true);
    try {
      await signInWithPopup(auth, googleProvider);
    } catch (err: any) {
      setError(translateError(err.code));
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="auth-container">
      <div className="auth-card" style={{ textAlign: 'center', gap: '24px' }}>
        <div className="auth-header">
          <h1 className="app-title" style={{ justifyContent: 'center', marginBottom: '8px' }}>
            Controle de Gastos
          </h1>
          <p className="auth-subtitle">
            Acesse suas finanças com segurança através da sua conta Google.
          </p>
        </div>

        {error && (
          <div className="auth-error" style={{ textAlign: 'left' }}>
            <AlertCircle size={18} style={{ flexShrink: 0 }} />
            <span>{error}</span>
          </div>
        )}

        <button 
          type="button" 
          className="btn btn-primary auth-submit-btn" 
          onClick={handleGoogleLogin}
          disabled={loading}
          style={{ 
            minHeight: '48px', 
            display: 'flex', 
            alignItems: 'center', 
            justifyContent: 'center', 
            gap: '12px',
            fontSize: '1rem',
            boxShadow: '0 4px 12px rgba(129, 140, 248, 0.25)'
          }}
        >
          {loading ? (
            <Loader className="spinner" size={20} />
          ) : (
            <>
              <svg 
                className="google-icon" 
                viewBox="0 0 24 24" 
                width="20" 
                height="20" 
                xmlns="http://www.w3.org/2000/svg"
                style={{ fill: '#1a1025' }}
              >
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="currentColor"/>
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="currentColor"/>
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z" fill="currentColor"/>
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z" fill="currentColor"/>
              </svg>
              <strong>Entrar com o Google</strong>
            </>
          )}
        </button>
      </div>
    </div>
  );
}
